...
function json_path([String] $json, [String] $jsonpath, [Hashtable] $substitutions) {
    ...
    if ($null -ne $substitutions) { $jsonpath = Invoke-VariableSubstitution -Entity $jsonpath -Parameters $substitutions -EscapeRegularExpression:($jsonpath -like '*=~*') }
    try {
...
function json_path_legacy([String] $json, [String] $jsonpath, [Hashtable] $substitutions) {
    ...
    $jsonpath.split('.') | ForEach-Object {
        ...
        if ($null -ne $substitutions) { $el = Invoke-VariableSubstitution -Entity $el -Parameters $substitutions }
